...
定义代码与项目结构发生变化时，`heroagent` 应优先检查哪些 wiki 文件，避免知识库长期失真。
...
- `want`、`plan`、`todo` 阶段通常只改工作流状态，不自动触发 wiki 判断
- 若讨论阶段已经明确改写了需求边界、架构约束、接口契约或数据模型，可例外补一次 `detect`
- 进入 `finish`、`achieve` 或显式 `wiki` 请求前，优先补一次正式同步判断
- 若只生成草稿，正式 wiki 仍视为未同步完成
- `detect` 应由 skill 在内部自动执行，不要求用户手动运行对应脚本
