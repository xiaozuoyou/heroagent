...
- 命中 `wiki` 维护意图时，可直接执行 `scripts/update_wiki_context.py`。

### 推进闭环

- `want`：将愿望收敛为可验收目标，不提前替用户展开计划。
  先按 `references/requirement-scoring.md` 做 10 分制评分，阈值 8 分。
  评分不足时进入 `目标澄清`，不直接输出正式目标卡片。
  若 `.heroagent/wiki/` 存在，先消费 `overview.md` 与 `arch.md` 的相关内容。
- `plan`：将目标拆成阶段路径，突出依赖、风险、关键路径。
  优先消费 wiki 中的模块结构、架构关系与约束信息。
- `todo`：将阶段路径落成可执行任务，任务应可完成、可验证、可排序。
  优先消费 wiki 中的模块、API、数据约束，再拆解任务。
- `focus`：只输出当前最值得关注的信息，不重复整段历史。
  可结合 wiki 和 `current-focus.md` 共同判断当前重点。
